...
export const BulkSongEditor: React.FC = () => {
  ...
  const exportToCSV = () => {
    const headers = ['Title', 'Composer', 'Language', 'ID'];
  ...
  const importFromCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const updates: Array<{ id: string; title: string; composer: string; language: string }> = [];
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const [title, composer, language, id] = matches.map(m => 
            m.replace(/^"(.*)"$/, '$1').replace(/""/g, '"').trim()
          );

          updates.push({ id, title, composer, language });
        }
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              <div className="text-sm font-medium text-indigo-900">
                {selectedIds.size} of {filteredSongs.length} selected
              </div>
              <button
                onClick={selectAll}
            ...
            {selectedIds.size > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  onClick={() => bulkUpdateLanguage('English')}
                  disabled={updating}
                  className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  English
                </button>
                <button
                  onClick={() => bulkUpdateLanguage('French')}
                  disabled={updating}
                  className="px-3 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  French
                </button>
                <button
                  onClick={() => bulkUpdateLanguage('Lingala')}
                  disabled={updating}
                  className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  Lingala
                </button>
                <button
                  onClick={() => bulkUpdateLanguage('Tshiluba')}
                  disabled={updating}
                  className="px-3 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  Tshiluba
                </button>
                <button
                  onClick={() => bulkUpdateLanguage('Swahili')}
                  disabled={updating}
                  className="px-3 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  Swahili
                </button>
                <button
                  onClick={() => bulkUpdateLanguage('Kikongo')}
                  disabled={updating}
                  className="px-3 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  Kikongo
                </button>
                <button
                  onClick={() => bulkUpdateLanguage('Portuguese')}
                  disabled={updating}
                  className="px-3 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  Portuguese
                </button>
                <button
                  onClick={() => bulkUpdateLanguage('Other')}
                  disabled={updating}
                  className="px-3 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  Other
                </button>
              </div>
            ...
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-6 py-3 text-left">
                  <button
                    onClick={selectedIds.size === filteredSongs.length ? selectNone : selectAll}
              ...
              {filteredSongs.map((song) => (
                <tr 
                  ...
                  className={`hover:bg-gray-50 transition-colors cursor-pointer ${
                    selectedIds.has(song.id) ? 'bg-indigo-50' : ''
                  }`}
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ol className="text-sm text-blue-800 space-y-1 list-decimal list-inside">
          ...
          <li>Open in Excel/Google Sheets and edit titles, composers, languages</li>
