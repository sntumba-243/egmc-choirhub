...
export const AdminEvents = () => {
  ...
  const handleDelete = async (e: React.MouseEvent, id: string, title: string) => {
    e.stopPropagation();
    // Check if event is global — only super admin can delete those
    const event = events.find(ev => ev.id === id);
    if (event?.is_global) {
      toast.error('Global events can only be deleted by super admin');
      return;
    }
  ...
  const handleResetRsvps = async (eventId: string, title: string) => {
    ...
    try {
      ...
      console.error('[ResetRSVPs] Failed:', e);
