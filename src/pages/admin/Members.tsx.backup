      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {Object.entries(voicePartCounts).map(([voice, count]) => {
          ...
          return (
            <div
              key={voice}
              className={`${colors[voice as keyof typeof colors]} rounded-xl p-4 border-2 text-center`}
            >
        ...
        <div className="space-y-3">
          {filteredMembers.map((member) => (
            <div
              ...
              className="bg-white rounded-xl shadow-sm hover:shadow-md active:scale-98 transition-all p-4 cursor-pointer border border-gray-100"
            >
